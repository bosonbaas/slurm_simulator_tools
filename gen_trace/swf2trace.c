...
#define MAX_REQ_NODES    30
#define CPUS_PER_NODE    8
#define MAX_WF_FILENAME_LEN     1024
static const char DEFAULT_OFILE[]    = "sample.trace"; //The output trace name should be passed as an argument on command line.
...
int main(int argc, char* argv[])
{
    ...
    nrecs = atoi(fgets(line, sizeof(line), file));
    printf("%d\n", nrecs);
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        while(p!=NULL){
            ...
            if(i==1) { printf("%s", p); strcpy(job_arr[idx].username, p); }
            if(i==2) { printf("%s", p); job_arr[idx].submit = 100 + atoi(p); }  // why submit cannot start from 0?
            if(i==3) { printf("%s", p); job_arr[idx].duration = atoi(p); }
            if(i==4) { printf("%s", p); job_arr[idx].wclimit = atoi(p); }
            if(i==5) { printf("%s", p); job_arr[idx].tasks = atoi(p); }
            if(i==6) { printf("%s", p); strcpy(job_arr[idx].qosname, p); }
            if(i==7) { printf("%s", p); strcpy(job_arr[idx].partition, p); }
            if(i==8) { printf("%s", p); strcpy(job_arr[idx].account, p); }
            if(i==9) { printf("%s", p); job_arr[idx].cpus_per_task = atoi(p); }
            if(i==10) { printf("%s", p); job_arr[idx].tasks_per_node = atoi(p); }
            if(i==11) { printf("%s", p); job_arr[idx].nice = atoi(p); }
            if(i==12) { printf("%s", p); strcpy(job_arr[idx].req_nodes, p); }
